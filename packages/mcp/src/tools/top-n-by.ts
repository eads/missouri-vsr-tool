...
// the result" instruction. Pattern: plain English first, then caveats,
// then a "Further reading:" line with raw URLs. No inline markdown
// links — the field is rendered as JSON.
const metricExplainer = (key: keyof typeof METRICS): string => {
  ...
  switch (key) {
    ...
    case "arrest_rate": {
      const plural =
        key === "search_rate" ? "searches" : key === "citation_rate" ? "citations" : "arrests";
      return `Plain English (surface BEFORE the numbers): this is the number of ${plural} per 100 stops over the window, per agency, ranked. Reported as per 100 stops, NOT a percentage — values can legitimately exceed 100 because a single stop can produce multiple events (one stop can produce multiple citations / arrests). When you describe these to a reader, say "12 ${plural} per 100 stops" not "12%". ${SHARED_RANKING_HINT} Further reading: call read_methodology() for the rate-vs-percentage distinction.`;
    }
    case "contraband_hit_rate":
      return `Plain English (surface BEFORE the numbers): of stops where officers searched, how often they actually found contraband — per 100 searches, per agency. A LOW hit rate paired with a HIGH search rate is the classic outcome-test pattern (see the disparity tool). Reported per 100, not as a percentage. Can exceed 100 if a single search recovered multiple distinct contraband categories. ${SHARED_RANKING_HINT} Further reading: call read_methodology() for the outcome-test framing and caveats.`;
    case "search_rate_minus_hit_rate":
      return `Plain English (surface BEFORE the numbers): search rate MINUS contraband hit rate, both per 100. A large negative number means an agency searches a lot but finds little — the outcome-test framing: many searches relative to how often something was actually found. This is one proxy for the Knowles/Persico/Todd 2001 outcome test; selection effects can produce the same pattern without any threshold difference, so do not call this "evidence of bias" on its own. ${SHARED_RANKING_HINT} Further reading: https://www.jstor.org/stable/2696570 (the original paper), https://openpolicing.stanford.edu/ (plain-English walkthrough); call read_methodology() for the full caveats.`;
    case "hispanic_stop_share":
    case "black_stop_share": {
      const race = key.replace("_stop_share", "").replace(/^./, (c) => c.toUpperCase());
      return `Plain English (surface BEFORE the numbers): the percentage of this agency's stops where officers recorded the driver as ${race}. This IS a percentage (0–100), part of a whole. It tells you the racial composition of the agency's stops — it does NOT tell you whether that's "high" without a population baseline. Pair with stop_share_vs_population_share for the comparison view, or with the disparity_index_all_stops ranking for a per-resident ratio. ${SHARED_RANKING_HINT} Further reading: call read_methodology() for the share-vs-rate distinction and the caveats around officer-perceived race.`;
    }
    case "resident_stop_share":
      return `Plain English (surface BEFORE the numbers): the percentage of this agency's stops that were of jurisdiction residents (vs. non-residents). Low share = an agency stopping mostly through-traffic — a flag worth following up on if you're reporting on revenue-from-outsiders dynamics in small-town courts. Only post-2020 reporting forms include this split, so smaller windows are more reliable. ${SHARED_RANKING_HINT} Further reading: call read_methodology() for the resident-vs-non-resident reporting history.`;
    case "disparity_index_all_stops":
      return `Plain English (surface BEFORE the numbers): a RATIO of the Black per-capita stop rate to the white non-Hispanic per-capita stop rate. 1.0 = parity; 2.0 = Black drivers stopped at twice the per-resident rate; below 1.0 = Black drivers stopped at a lower rate than white. Strong caveat: this is a stops-over-population framing — activists cite it as proof of biased policing, police argue the population denominator omits through-traffic, commuter patterns, and other context. Statewide or county-rolled disparity indexes can flip sign once you break them out by agency; always report at the agency-year level. ${SHARED_RANKING_HINT} Further reading: https://en.wikipedia.org/wiki/Simpson%27s_paradox (why aggregates can flip sign vs. per-agency); call read_methodology() for the full caveat list.`;
    case "total_stops":
      return `Plain English (surface BEFORE the numbers): just the count of stops the agency filed over the window. MSHP and the largest municipal departments will always lead this — total volume isn't a per-capita or per-officer measure, so don't read "rank #1" as "most aggressive." Useful for sizing context next to a rate-based ranking. Further reading: call read_methodology() for how to use volume as context for rate-based rankings.`;
    default:
      return `Plain English: this ranks agencies by ${key}. Further reading: call read_methodology() for the definition and caveats.`;
  }
};
