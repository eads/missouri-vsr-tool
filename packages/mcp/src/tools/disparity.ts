...
const DisparityInput = z.object({
  ...
  year_range: z
    ...
    .optional()
    .describe("Inclusive [start, end] window. Default is the four most recent years in the loaded release (e.g. 2022–2025); 2020 is excluded by default because the AG's published 2020 data has unreconciled anomalies. Pass year_range explicitly to widen, and the response will include a data_quality_warnings entry if 2020 is in range."),
  county: z
    .string()
    .optional()
    .describe(
...
const disparityHandler = async (raw: unknown) => {
  ...
  // Latest year comes from the loaded release (2025 as of v2.2), not a
  // hardcoded constant that goes stale on every data release (#219).
  const [start, end] = args.year_range ?? defaultWindow(await getLatestYearWithData(), 4);
  ...
  const payload = {
    ...
    method_explainer:
      args.comparison_type === "outcome_test"
        ? "Plain English (surface this BEFORE the numbers, don't just paste the ratios): we're asking two questions side by side — (1) is one race searched MORE often per stop, and (2) when officers do search, do they find contraband at the SAME rate? If officers used the same suspicion threshold for everyone, the hit rates would be roughly equal at the margin. A pattern where one group is searched more often AND contraband is found less often is consistent with officers using a lower threshold for that group — but it does NOT prove discrimination on its own. Selection effects (different driving populations, road types, time-of-day patterns) can produce identical numbers. Further reading: https://openpolicing.stanford.edu/ (plain-English walkthrough of the test), https://en.wikipedia.org/wiki/Selection_bias (why a pattern like this isn't proof); call read_methodology() for the formal Knowles/Persico/Todd 2001 statement."
        : args.comparison_type === "search_rate"
          ? "Plain English (surface BEFORE the numbers): how many searches per 100 stops, broken down by the driver's officer-perceived race. The ratio compares each race to white non-Hispanic drivers — above 1.0 means searched at a higher rate, below 1.0 lower. This describes WHAT happened, not WHY: selection effects (different populations being stopped in the first place, different stop contexts) can produce these patterns without any threshold difference. Further reading: https://openpolicing.stanford.edu/ (plain-English walkthrough of how to interpret search-rate disparities responsibly); call read_methodology() for the formal caveats."
          : "Plain English (surface BEFORE the numbers): when officers searched a stopped driver, how often did they actually find contraband? Broken down by the driver's officer-perceived race, with a ratio vs. white non-Hispanic. A LOW hit rate combined with a HIGH search rate (see the outcome_test view) is the classic outcome-test pattern: lots of searches that don't find anything, more often for one group than another. By itself, a hit-rate ratio is not proof of bias. Further reading: https://openpolicing.stanford.edu/ (context); call read_methodology() for the formal treatment.",
