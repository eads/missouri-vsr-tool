...
**Officer-perceived race and self-identification move independently — the "Other" category is the clearest example.** Stop records carry the race an officer perceived; the ACS population baselines used for stop rates and disparity carry how residents self-identify. "Other" is the fastest-growing race category in Missouri's ACS — it more than doubled, from roughly 2.8% to 7.6% of the 16-and-over population between 2016 and 2025, a real shift in demographics and/or self-identification. Over the same span, the share of stops where an officer recorded the driver as "Other" rose only slightly (roughly 0.8% to 1.6%). So when you compare a stop share to a population share for any race — and especially for "Other" — you are comparing an officer's perception to a resident's self-report, not the same quantity measured twice. A large gap for "Other" is expected and is mostly about that perception-vs-self-ID difference, not necessarily under-stopping.
...
- **Disparity index**: \`stop_rate_minority / stop_rate_white_non_hispanic\`. A value of 1.0 means parity with white drivers; values above 1.0 mean the minority race is stopped at a higher rate per resident. The denominator is white non-Hispanic by convention. Multiple variants exist for different population baselines (decennial vs. ACS; all-stops vs. resident-stops); call \`read_schema()\` for the full list. **The population baseline changed over time, and the index is sensitive to it:** report years 2011–2019 use the 2010 decennial census, while 2020 onward uses ACS 5-year estimates. A jump that straddles the 2019→2020 boundary partly reflects that change in how the population was *estimated*, not only a change in policing — the state marks these population-estimate changes with vertical lines in Figure 4 of its statewide report. **Provenance and coverage are uneven:** the state itself published \`disparity-index--all-stops\` (and \`--resident-stops\`) only through report year 2021 and then stopped; the \`-acs\` / \`-dec\` variants for 2022 and 2025 are this project's own population-based reconstructions (ACS 5-year vs. 2020 decennial denominators), and no index exists for 2023–2024 in the current release. Label which one you are quoting. **There is no statewide disparity index:** the \`missouri-all-agencies\` rollup carries none (the pipeline's rollup rows for this family were sums of per-agency ratios and are dropped by this server), and a per-capita stop rate is not meaningful for an all-agencies aggregate anyway — jurisdictions overlap and through-traffic has no population denominator. **Warning:** statewide or county-rolled disparity indexes can flip direction once you break them out by agency — see [Simpson's paradox](https://en.wikipedia.org/wiki/Simpson%27s_paradox). Always report disparity at the agency-year level alongside any aggregate.
...
- **Defaults**: \`trend\`, \`disparity\`, \`agency_summary\`, and \`stop_share_vs_population_share\` default their multi-year windows to the **four most recent years in the loaded release, floored at 2021** (2022–2025 as of data release v2.2) rather than reaching back into 2020.
...
### Structural breaks: discontinuities to know before comparing across years

Several real-world and reporting changes create step-changes in the series. A break that straddles one of these may be the story, or it may be an artifact — treat such comparisons with care and name the discontinuity.

- **2009–2017 — odor-based search reason not itemized.** The "smell of drugs/alcohol" search reason is not reported separately in 2009–2017; it appears as its own line only from 2018 on. A zero or missing odor series before 2018 is a reporting gap, not an absence of such searches.
- **2019 → 2020 — reporting-form revision + 2020 anomalies.** The filing form changed across this boundary (see the 2020 floor above). Some metrics are era-specific, and the published 2020 data carries unreconciled anomalies.
- **2020 — population-baseline switch.** Population denominators moved from the 2010 decennial census (report years 2011–2019) to ACS 5-year estimates (2020 onward). Anything per-capita (stop rate, disparity index) is sensitive to this; a change across the boundary partly reflects the denominator switch, not only policing.
- **Late 2022 → 2023 — recreational cannabis legalized.** Missouri voters approved Amendment 3 in November 2022; possession became legal on December 8, 2022, and licensed sales began in early 2023. The 2023 report shows the consequences directly: odor-based ("smell of drugs/alcohol") searches collapse, and contraband-found (hit) rates shift sharply — overall hit rates roughly halve and the racial ordering moves. **This is a legal/behavioral change, not a documented reporting-form change** — do not attribute the 2023 break to a form revision. It is the single most important discontinuity for any search- or contraband-related trend spanning 2022–2023.
- **2024 → 2025 — agency-coverage churn.** About 110 agencies that filed in 2023 are missing from the 2024 report; roughly 90 of them return in 2025. Statewide **absolute counts** (total stops, searches, arrests) are therefore unreliable across the 2023–2025 window — lean on shares, rates, and long-run trends instead. The 2025 rebound may be linked to Governor Kehoe's Blue Shield program (enacted early 2025), whose funding-eligibility criteria include "compliance with Missouri crime reporting and traffic stop data requirements." See Further Reading.
...
- **Ask the agency — and the AG's press office.** For *why* an agency's stop reasons, rates, or types look the way they do, the answer is almost always at the **individual department**. The Missouri AG's office explicitly **declines to speculate** on the reasons behind stop patterns and directs reporters to the agencies, so put the trend you observed to the department directly. The AG's press contact is **press@ago.mo.gov** — useful for questions about the report itself, the ingest pipeline, or statewide context, but expect to be pointed back to the departments for agency-specific explanations.
...
- Missouri Blue Shield program (Gov. Kehoe, launched 2025) — grant funding for local law enforcement whose eligibility criteria include compliance with Missouri crime-reporting and traffic-stop-data requirements; a plausible driver of the 2025 rebound in reporting agencies — https://governor.mo.gov/press-releases/archive/governor-kehoe-announces-launch-missouri-blue-shield-program-recognize
